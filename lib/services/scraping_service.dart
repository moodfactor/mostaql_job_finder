...
class ScrapingService {
  ...
  Future<List<Job>> fetchJobs({String? category, int page = 1}) async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var element in jobElements) {
        ...
        final offerCountElement = element
            .querySelector('li.text-muted i.fa-ticket')
            ?.parent;
        final offerCountString = offerCountElement?.text.trim() ?? '0';
        final offerCount =
            int.tryParse(offerCountString.replaceAll(RegExp(r'[^0-9]'), '')) ??
            0;
        jobs.add(
          Job(
            title: title,
  ...
  Future<Job> fetchJobDetails(String jobUrl) async {
    ...
    final descElement = document.querySelector(
      'div#projectDetailsTab div.text-wrapper-div.carda__content',
    );
    if (descElement != null) {
    ...
    final offerCountElement = document.querySelector(
      '#project-bids > .heada > .heada__title',
    );
    ...
    List<String> skills = [];
    final skillElements = document.querySelectorAll(
    ...
    if (employerCard != null) {
      employerName = employerCard
          .querySelector('h5.postcard__title.profile__name bdi')
          ?.text
          .trim();
      employerProfession = employerCard
          .querySelector('ul.meta_items li a')
          ?.text
          .trim();

      final metaItems = employerCard.querySelectorAll(
        'table.table-meta tbody tr',
      );
      for (var item in metaItems) {
        final labelElement =
            item.querySelector('td:first-child span') ??
        ...
        if (label != null && value != null) {
          if (label.contains('تاريخ التسجيل')) {
            employerRegistrationDate =
                valueElement?.querySelector('time')?.text.trim() ?? value;
          } else if (label.contains('معدل التوظيف')) {
            employerHiringRate =
                valueElement?.querySelector('label')?.text.trim() ?? value;
